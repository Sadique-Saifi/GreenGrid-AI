"""
GreenGrid AI — Solar Generation Model Training Script
Trains a Random Forest Regressor to predict solar power output (solar_generation).
Saves trained model artifact to ML/models/solar_model.pkl.
"""
...
from preprocessing import prepare_solar_datasets, SOLAR_FEATURES
...
def train_solar_model(models_dir: str = DEFAULT_MODELS_DIR):
    print("=" * 60)
    print("GreenGrid AI -- Training Solar Generation Model (solar_generation)")
    print("=" * 60)

    X_train, y_train, X_val, y_val, X_test, y_test = prepare_solar_datasets()
    ...
    print(f"\n[OK] Solar model saved successfully to: {model_path}")
    print("=" * 60)
    return artifact
